...
def format_add(item):
    add = USER_SHOP + CONTROL_BOLD + "[" + CONTROL_COLOR + colors.LIGHT_GREEN + "+" + CONTROL_NORMAL + CONTROL_BOLD + "]" + CONTROL_NORMAL + " ADDED " + str(
        item) + " to your Inventory. Check .inventory for more infos."
    return add


def format_rem(item):
    rem = USER_SHOP + CONTROL_BOLD + "[" + CONTROL_COLOR + colors.RED + "-" + CONTROL_NORMAL + CONTROL_BOLD + "]" + CONTROL_NORMAL + " REMOVED " + str(
...
def f_catalogue(bot , catalogue : dict , colour = "cyan"):
    ...
    for item in catalogue:
        bot.say(f_item("ITEM: ", colour) + item +f_item(" COST: " , colour) + str(catalogue[item]["cost"]) + f_item(" STOCK: ", colour) +
                        str(catalogue[item]["quantity"]) + f_item(" DESCRIPTION: ", colour) + str(catalogue[item]["description"] +
                                                                                 f_item(" EFFECT: ", colour) + str(catalogue[item]["effect"])))
    bot.say(CONTROL_BOLD + CONTROL_ITALIC + "If you want to buy something, use .buy <catalogue_name> <item_name> (<amount>, default = 1). E.g. : .buy food ice_cream 2")

def f_inventory(bot , catalogue : dict , colour = "white"):
    bot.notice(CONTROL_BOLD + CONTROL_ITALIC + "Here's your Inventory!")
    for item in catalogue:
        bot.notice(f_item("ITEM: ", colour) + item +f_item(" COST: " , colour) + str(catalogue[item]["cost"]) + f_item(" STOCK: ", colour) +
                        str(catalogue[item]["quantity"]) + f_item(" DESCRIPTION: ", colour) + str(catalogue[item]["description"] +
                                                                                 f_item(" EFFECT: ", colour) + str(catalogue[item]["effect"])))
...
def f_item(name , colour = "cyan"):
    colori = {
        "red" : CONTROL_COLOR + colors.RED + "%s" +CONTROL_NORMAL,
...
def info_item(bot,
    ...
    if item_name == "/":
        ...
        return
    if item_name not in catalogue:
        bot.say(SHOP + "The item " + f_item(item_name , "cyan") +
                " IS NOT in the catalogue " + f_item(cat_sec , "cyan")+ ".")
        return

    bot.say(SHOP + "Item INFO: " + f_item(item_name , "cyan") + " in catalogue " + f_item(cat_sec , "cyan") +
            CONTROL_ITALIC + ":   " +str(catalogue[item_name]))
...
def del_item(bot,
    ...
    bot.say(SHOP + "Item " + f_item(item_name , "cyan") + " successfully " + f_item("DELETED" , "red") +
            " from the catalogue " + f_item(cat_sec , "cyan") + ".")
    if len(catalogue) == 0:
...
def item_add(bot , trigger):
    ...
    if not trigger.group(2):
        ...
        bot.notice("You can omit EFFECT and DESCRIPTION by writing '/' instead. E.g.: .item add food,pizza,5,10,/,typical_italian_food" , log_chan)
...
def item_usertake(bot , trigger):
    ...
    if amount == "*":
        ...
        bot.say(SHOP + f_item("REMOVED " , "red") + str(amount) +" "+ f_item(item_name , "cyan") +
            " from the Inventory of " + f_item(trigger.group(3) , "cyan") + ".")
...
def shop(bot , trigger):
    ...
    if not trigger.group(3):
        bot.say(USER_SHOP + "Welcome to TriShop! We have the following Catalogues available to visit: " +
                f_item("FOOD " , "red"))
...
def buy(bot , trigger):
    ...
    if catalogue == {}:
        bot.say(f_item("ERROR: " , "red")+ "Catalogue " + f_item(cat_name) + " doesn't exist. Wrong format? Remember to replace spaces with '_'")
        return
    if item_name.lower() not in catalogue:
        bot.say(f_item("ERROR: " , "red")+"Item " + f_item(item_name) + " isn't in "+ f_item(cat_name)+ ". Wrong format? Remember to replace spaces with '_'")
        return
    ...
    if catalogue[item_name]["quantity"] == 0:
        bot.say(USER_SHOP + "The item " + item_name + " is temporarily over. Contact an admin if you'd like more added!")
        return
    if user_money < cost:
        bot.say(USER_SHOP + "I'm really sorry, but you don't have enough money.")
        return
    if amount > catalogue[item_name]["quantity"]:
        bot.say(USER_SHOP + "I'm really sorry, but the shop only has " + f_item(str(catalogue[item_name]["quantity"]) , "pink") +
                " while you requested " + f_item(str(amount) , "pink") +".")
        return

    if item_name not in user_inventory:
        user_inventory[item_name] = {"cost": catalogue[item_name]["cost"], "quantity": amount,
                                     "effect": catalogue[item_name]["effect"], "description": catalogue[item_name]["description"]}
    else:
    ...
    bot.say(format_add(item_name) , trigger.nick)
