...
def format_add(money, reason):
    add = BANK + CONTROL_BOLD + "[" + CONTROL_COLOR + colors.LIGHT_GREEN + "+" + CONTROL_NORMAL + CONTROL_BOLD + "]" + CONTROL_NORMAL + " ADDED " + str(
        money) + coins() + " to your Bank Account. Reason: " + CONTROL_ITALIC + reason
    return add


def format_rem(money, reason):
    rem = BANK + CONTROL_BOLD + "[" + CONTROL_COLOR + colors.RED + "-" + CONTROL_NORMAL + CONTROL_BOLD + "]" + CONTROL_NORMAL + " REMOVED " + str(
        money) + coins() + " from your Bank Account. Reason: " + CONTROL_ITALIC + reason
    return rem
...
def give(bot, trigger):
    ...
    bot.notice(BANK + trigger.nick + ": correctly added " + str(trigger.group(4)) + coins() + "to " + trigger.group(
        3) + "'s Bank Account.", trigger.nick)
    bot.say(
        BANK + trigger.nick + " gave " + str(trigger.group(4)) + coins() + "to " + trigger.group(3) + " succesfully.",
        log)
...
def take(bot, trigger):
    ...
    bot.notice(BANK + trigger.nick + ": correctly removed " + str(trigger.group(4)) + coins() + "from " + trigger.group(
        3) + "'s Bank Account.", trigger.nick)
    bot.say(
        BANK + trigger.nick + " took " + str(trigger.group(4)) + coins() + "from " + trigger.group(3) + " succesfully.",
        log)
...
def transfer(bot, trigger):
    ...
    if not trigger.group(5) or trigger.group(5) == "*":
        ...
        bot.say(BANK + trigger.nick + " transferred " + str(money_from) + coins() + "from " + trigger.group(
            3) + " to " + trigger.group(4) + " succesfully.", log)
