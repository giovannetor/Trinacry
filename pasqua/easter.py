...
class easteregg:  # everything is inside a single little class.
    ...
    def adminegg(self, bot, trigger):  # for admins only. in case of need, an admin can manually give an egg to a user.
        try:
            ...
            bot.notice(strings["admineggself"] % (trigger.nick),
                       trigger.nick)  # using .giveegg without argument will result
...
@module.commands(
    "uovodipasqua")  # this command might change it's syntax in the future (maybe a code to find trough some
